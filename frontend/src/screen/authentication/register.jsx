...
const Register = () => {
  ...
  useEffect(() => {
    const updateBtn = () => {
      if (
        ...
        registerData.confPassword.trim() != ""
      ) {
        setDisableBtn(false);
      } else {
        setDisableBtn(true);
  ...
  const registerUser = async (e) => {
    ...
    if (registerData.confPassword === registerData.password) {
      if (
        ...
        registerData.password.trim().length > 12
      ) {
        showAlert(
          "Password should be between 6 to 12 character. Password should not contain any white spaces.",
          false
        );
      } else {
        await createUser(
  ...
  return (
    <>
      ...
      <div className="flexCenter authContainer">
        ...
        >
          ...
          <div className="flex handlers">
            ...
            <Link to="/auth/verifyEmail" className={loader ? "none" : ""}>
              Verify E-Email??
            </Link>
          </div>
        </form>
      ...
      <Link
        to="/auth/login"
        className={loader ? "none redirectAuthPage" : "redirectAuthPage"}
      >
        <p>login</p>
      </Link>
